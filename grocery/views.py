...
def Signup(request):
    if request.method == 'POST':
        ...
        if check:
            ...
            context = {"mail_sent": 'Registered Succesfully'}
            return render(request, "signup.html", context)
...
def Login(request):
    ...
    if request.method == "POST":
        ...
        chk = User.objects.filter(username=u).exists()

        if chk :
            uid = User.objects.filter(username=u).first()
            validate = Profile.objects.filter(user=uid).first()
            validate.verified="True"
            if validate.verified != "True":
